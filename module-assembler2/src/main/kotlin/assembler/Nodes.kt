...
val AstNode.printableString: String get() = when(this) {
	...
	is VarDNode        -> "var ${symbol.name} ${width.dString} ${components.joinToString { it.printableString }}"
	is VarResNode      -> "var ${symbol.name} res $size"
}



private val Width.dString get() = when(this) {
	Width.BIT8  -> "db"
	Width.BIT16 -> "dw"
	Width.BIT32 -> "dd"
	Width.BIT64 -> "dq"
...
private val BinaryNode.printableString get() = buildString {
	append('(')
...
private val MemNode.printableString get() = buildString {
	if(width != null) {
		append(width.string)
		append(' ')
	}
...
private val InstructionNode.printableString get() = buildString {
