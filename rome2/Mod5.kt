...
/**
 * - Small buffs to units to justify upgrading them
 * - Small buffs to ranged unit reload, ammunition, and range
 * - Buffs to weapons, shields, and armours
 * - Garrison buffs
 * - Tech buffs
 * - Small building effect buffs (but not gdp)
...
object Mod5 {
	...
	private fun modifyUnits() {
		...
		Shields.SCUTUM_MARIAN.mod    { defence = 40; armour = 45; block = 55 } // 25 + 15  40 + 5   50 + 5
		Shields.SCUTUM_IMPERIAL.mod  { defence = 50; armour = 55; block = 60 } // 25 + 25  40 + 15  50 + 10
	...
	private fun modifyBuildings() {
		...
		Buildings.ROMAN_CITY_CIVIL_3.apply {
			effect(GDP_SUBSISTENCE, 400) // 400
			effect(GDP_MOD_ALL, 15) // 10 + 5
			effect(GROWTH, 6) // 7 - 1
		}
		Buildings.ROMAN_CITY_CIVIL_4.apply {
			effect(GDP_SUBSISTENCE, 500) // 500
	...
	private fun modifyGarrisons() {
		...
		//GarrisonGroup("rom_roman_strong_ranged", emptyList()).mod { unit(Units.AUX_PELTASTS) }

		Buildings.ROMAN_BARRACKS_MAIN_2.setGarrison(
			GarrisonGroups.ROMAN_BASIC_MELEE to 2,
			GarrisonGroups.ROMAN_MEDIUM_MELEE to 2
		)
		Buildings.ROMAN_BARRACKS_MAIN_3.setGarrison(
			GarrisonGroups.ROMAN_MEDIUM_MELEE to 2,
			GarrisonGroups.ROMAN_STRONG_MELEE to 2
		)
		Buildings.ROMAN_BARRACKS_MAIN_4.setGarrison(
			GarrisonGroups.ROMAN_MEDIUM_MELEE to 2,
			GarrisonGroups.ROMAN_STRONG_MELEE to 2,
			GarrisonGroups.ROMAN_ELITE_MELEE to 2
		)

		Buildings.ROMAN_BARRACKS_AUX_2.setGarrison(
			GarrisonGroups.ROMAN_MEDIUM_MELEE to 2,
			GarrisonGroups.ROMAN_MEDIUM_RANGED to 2
		)
		Buildings.ROMAN_BARRACKS_AUX_3.setGarrison(
			GarrisonGroups.ROMAN_STRONG_MELEE to 3,
			GarrisonGroups.ROMAN_MEDIUM_RANGED to 3
		)
		Buildings.ROMAN_BARRACKS_AUX_4.setGarrison(
			GarrisonGroups.ROMAN_STRONG_MELEE to 2,
			GarrisonGroups.ROMAN_ELITE_MELEE to 2,
			GarrisonGroups.ROMAN_MEDIUM_RANGED to 4
		)

		Buildings.ROMAN_CITY_1.setGarrison(
			GarrisonGroups.ROMAN_LEVY_MELEE to 6,
			GarrisonGroups.ROMAN_BASIC_RANGED to 2
		)
		Buildings.ROMAN_CITY_CIVIL_2.setGarrison(
			GarrisonGroups.ROMAN_LEVY_MELEE to 6,
			GarrisonGroups.ROMAN_BASIC_MELEE to 4,
			GarrisonGroups.ROMAN_BASIC_RANGED to 3
		)
		Buildings.ROMAN_CITY_CIVIL_3.setGarrison(
			GarrisonGroups.ROMAN_LEVY_MELEE to 6,
			GarrisonGroups.ROMAN_BASIC_MELEE to 6,
			GarrisonGroups.ROMAN_MEDIUM_RANGED to 3
		)
		Buildings.ROMAN_CITY_CIVIL_4.setGarrison(
			GarrisonGroups.ROMAN_BASIC_MELEE to 6,
			GarrisonGroups.ROMAN_MEDIUM_MELEE to 6,
			GarrisonGroups.ROMAN_MEDIUM_RANGED to 4
		)
		...
		Buildings.ROMAN_CITY_GARRISON_3.setGarrison(
			GarrisonGroups.ROMAN_MEDIUM_MELEE to 8,
			GarrisonGroups.ROMAN_MEDIUM_RANGED to 4
		)
		Buildings.ROMAN_CITY_GARRISON_4.setGarrison(
			GarrisonGroups.ROMAN_STRONG_MELEE to 8,
			GarrisonGroups.ROMAN_ELITE_MELEE to 2,
			GarrisonGroups.ROMAN_MEDIUM_RANGED to 6
		)

		for(building in Buildings.ROMAN_TEMPLES) {
			if(building.isLevel2) building.garrison(GarrisonGroups.ROMAN_BASIC_MELEE)
			if(building.isLevel3) building.garrison(GarrisonGroups.ROMAN_MEDIUM_MELEE)
			if(building.isLevel4) building.garrison(GarrisonGroups.ROMAN_STRONG_MELEE)
			if(building.isLevel5) building.garrison(GarrisonGroups.ROMAN_ELITE_MELEE)
		}
		...
		Buildings.ROMAN_TOWN_TRADE_4.setGarrison(
			GarrisonGroups.ROMAN_STRONG_MELEE to 6,
			GarrisonGroups.ROMAN_ELITE_MELEE to 1,
			GarrisonGroups.ROMAN_MEDIUM_RANGED to 5,
		)
		...
		for(building in Buildings.ROMAN_TOWNS) {
			...
			if(building.isLevel1) building.setGarrison(
				GarrisonGroups.ROMAN_LEVY_MELEE to 4,
				GarrisonGroups.ROMAN_BASIC_RANGED to 2
			)
			...
			if(building.isLevel3) building.setGarrison(
				GarrisonGroups.ROMAN_LEVY_MELEE to 8,
				GarrisonGroups.ROMAN_BASIC_MELEE to 4,
				GarrisonGroups.ROMAN_MEDIUM_RANGED to 3
			)
	...
	private fun modifyTechs() {
		RomanTechs.PROFESSIONAL_SOLDIERY.cost *= 3
